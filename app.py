        ...
        for idx, jlabel in enumerate(jornadas):
            ...
            with tab:
                ...
                for jogo in rows:
                    ...
                    # Visible ONLY once the match has started (`disabled`, i.e.
                    # kickoff has passed) or already has a locked-in result
                    # (`jogo_terminado`). Before kickoff it stays hidden so no
                    # one can see — or copy — predictions for a match that has
                    # not happened yet. Points are shown only when a result
                    # exists; for a started-but-unfinished match they are omitted.
                    jogo_terminado = (
                        jogo.get("golos_casa_real") is not None
                        and jogo.get("golos_fora_real") is not None
                    )
                    if jogo_terminado or disabled:
                        outras = [
